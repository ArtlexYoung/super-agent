...
class SkillUpdateTests(unittest.TestCase):
    ...
    def test_testing_does_not_activate_candidate_and_failure_blocks_apply(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(ValueError, "did not pass"):
                updater.apply_skill_change(change.change_id)
            self.assertFalse((agent._setup.create_event_store("alice").private_root / "skills").exists())
    ...
    def test_apply_and_undo_restore_project_skill(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            applied = updater.apply_skill_change(change.change_id)
            user_path = agent._setup.create_event_store("alice").private_root / "skills" / "prompt" / "writer"
            ...
            event_types = [
                event.event_type
                for event in agent._setup.create_event_store("alice").read_events("skill_change")
            ]
            self.assertEqual(
                [
            ...
            action_events = agent._setup.create_event_store("alice").read_events("action")
