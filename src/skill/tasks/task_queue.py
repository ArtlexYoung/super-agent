...
class TaskQueue:
    ...
    def read_results(self) -> dict[str, object]:
        return {"agent_tasks": self.list_tasks(), "agent_groups": self.list_groups()}
    ...
    def _create_task(self, arguments: dict[str, object]) -> dict[str, object]:
        ...
        estimates = tuple(read_optional_estimated_tokens(arguments, name) for name in ("estimated_output_tokens", "estimated_cache_creation_tokens", "estimated_cache_read_tokens"))
        with self._condition:
            if self._closed:
    ...
    def _dispatch_task(self, arguments: dict[str, object]) -> dict[str, object]:
        ...
        with self._condition:
            ...
            choice = self._select_agent_locked(task, requested_agent)
            queued = self._queue_selected_task_locked(task, choice)
            return {"task": queued.to_dict(), **choice.to_dict(), "rotation_limited": (self.settings.agent_selection == "rotate" and choice.candidate_count < 2)}
    ...
    def _consume_task(self, task_id: str) -> None:
        try:
            with self._condition:
                ...
                record_options = self.settings.record_options_for_task(self._started_task_count)
                running = self._transition_locked(task, "running", record_mode=record_options.mode, record_task_number=self._started_task_count, attempt_count=task.attempt_count + 1, retry_after_seconds=None)
    ...
    def _wait_for_tasks(self, arguments: dict[str, object]) -> dict[str, object]:
        ...
        started = monotonic()
        self.record_event("agent_task.wait.started", {"trigger": trigger, "task_ids": list(task_ids), "requested_wait_seconds": requested_wait, "wait_seconds": wait_seconds})
        with self._condition:
    ...
    def _retry_unavailable_locked(self, task: QueuedTask, error: Exception) -> None:
    ...
    def _matching_tasks_locked(self, trigger: str, task_ids: tuple[str, ...]) -> list[str]:
        ...
        if trigger == "any_task_finished":
            return [task.task_id for task in unseen if task.status in TERMINAL_TASK_STATUSES]
    ...
    def _transition_locked(self, task: QueuedTask, status: str, **changes: object) -> QueuedTask:
...
def create_task_queue(tools: dict[str, dict[str, object]], subagents: list[dict[str, object]], run_subagent: Callable[..., dict[str, object]], record_event: Callable[[str, dict[str, object]], object], record_result: Callable[[dict[str, object]], None] | None = None, create_shared_context: Callable[[str, str], dict[str, object]] | None = None) -> TaskQueue | None:
    ...
    return TaskQueue(TaskQueueSettings.from_dict(tools["agent_tasks"]), subagents, run_subagent, record_event, record_result, group_settings, create_shared_context)
