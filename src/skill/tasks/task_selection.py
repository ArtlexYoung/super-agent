...
class AgentSelector:
    ...
    def _rank(self, agent: dict[str, object], task: QueuedTask, active: dict[str, int]) -> tuple[float, float, float]:
        cost = choose_dispatch_model(agent.get("models"), task.purpose, task.required_features, task.token_counts()).cost
