...
from core.models import RunIdentity, read_int, read_text, reject_unknown_fields
from core.provider import Message, ToolDefinition
from skill.discovery.catalog import DisclosureRecorder, ProgressiveDisclosureCore, SkillDisclosure, SkillReference
...
def _create_task_policy(disclosure: SkillDisclosure, expected_type: str) -> TaskPolicy:
    ...
    mode = read_text(data["mode"], f"{expected_type} mode").lower()
    if mode not in WORKFLOW_MODES:
        raise ValueError(f"unknown workflow mode: {mode}")
    reject_unknown_fields(data, {"mode", "max_steps", "tools"}, f"{expected_type} Skill settings")
    instruction = read_text(disclosure.read_instructions().content, f"{expected_type} Skill instructions")
...
class SkillAction:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.resource, str) and not self.resource.strip(): raise ValueError("Skill action resource cannot be empty")
        object.__setattr__(self, "resource", read_text(self.resource, "Skill action resource"))
        if self.resource_argument is not None:
            if isinstance(self.resource_argument, str) and not self.resource_argument.strip(): raise ValueError("Skill action resource argument cannot be empty")
            object.__setattr__(self, "resource_argument", read_text(self.resource_argument, "Skill action resource argument"))
    ...
    def resolve_resource(self, arguments: ToolArguments) -> str:
        ...
        selected = "" if isinstance(value, bool) or not isinstance(value, str | int) else str(value).strip()
...
class SkillContext:
    ...
    def require_store(self, feature: str = "Skill handler") -> EventStore:
        if self.store is None: raise ValueError(f"{feature} requires Runtime storage")
        return self.store

    def require_action_executor(self) -> Callable[[ActionRequest, Callable[[], object]], object]:
        if self.execute_action is None: raise ValueError("Skill handler requires a Runtime action executor")
...
def validate_skill_result(result: object) -> None:
    if not isinstance(result, SkillUse): raise TypeError("SkillHandler.handle_skill must return SkillUse")
    for name, expected in (("model_context", Skill), ("task_completed_action", SkillAction), ("source", SkillReference)):
        value = getattr(result, name)
        if value is not None and not isinstance(value, expected): raise TypeError(f"SkillUse.{name} must be {expected.__name__} or None")
    for name in ("build_prompt_context", "record_task_completed", "start_session"):
        value = getattr(result, name)
        if value is not None and not callable(value): raise TypeError(f"SkillUse.{name} must be callable or None")
    _validate_skill_tools(result.tools)
    if result.task_policy is not None:
        _validate_task_policy(result.task_policy)
    if (result.record_task_completed is None) != (result.task_completed_action is None): raise TypeError("A Skill completion callback must declare one SkillAction")
    references = result.included_skills
    if not isinstance(references, tuple) or not all(isinstance(reference, SkillReference) for reference in references): raise TypeError("SkillUse.included_skills must contain SkillReference values")
    if len({reference.key for reference in references}) != len(references): raise ValueError("SkillUse.included_skills cannot contain duplicates")
...
def _validate_task_policy(policy: object) -> None:
    if not isinstance(policy, TaskPolicy): raise TypeError("SkillUse.task_policy must be a TaskPolicy or None")
    read_text(policy.name, "TaskPolicy.name")
    if policy.mode not in WORKFLOW_MODES: raise ValueError(f"TaskPolicy.mode is invalid: {policy.mode}")
    read_text(policy.instruction, "TaskPolicy.instruction")
...
def _validate_skill_tool(tool: object) -> None:
    if not isinstance(tool, SkillTool): raise TypeError("SkillUse.tools must contain SkillTool values")
    if not isinstance(tool.name, str) or not tool.name.strip() or not isinstance(tool.description, str) or not tool.description.strip(): raise ValueError("Skill tool name and description cannot be empty")
    if not isinstance(tool.properties, dict) or not callable(tool.handler): raise TypeError(f"Skill tool is invalid: {tool.name}")
    if not isinstance(tool.required, tuple) or not all(isinstance(name, str) and name in tool.properties for name in tool.required): raise ValueError(f"Skill tool required names are invalid: {tool.name}")
    if not isinstance(tool.action, SkillAction): raise TypeError(f"Skill tool is missing an action: {tool.name}")
    if tool.result_kind is not None and (not isinstance(tool.result_kind, str) or not tool.result_kind.strip()): raise ValueError(f"Skill tool result_kind is invalid: {tool.name}")
    argument = tool.action.resource_argument
    if argument is not None and argument not in tool.properties: raise ValueError(f"Skill tool action argument is not declared: {tool.name}.{argument}")
...
def create_default_skill_handlers(mcp_servers: McpServers | None = None) -> SkillHandlers:
    ...
    return SkillHandlers(tuple(create_builtin_skill_handlers(mcp_servers or McpServers())))
...
def create_progressive_skill_disclosure(config: CommonConfig, *, store: EventStore | None = None, identity: RunIdentity | None = None, record_disclosures: bool | None = None, include_freshness: bool = True) -> ProgressiveDisclosureCore:
    ...
    if store is not None and include_freshness and "freshness" not in config.agent.disabled_skills:
        from skill.learning.freshness import calculate_skill_freshness, load_freshness_rules
...
def load_configured_freshness_rules_if_enabled(config: CommonConfig, *, store: EventStore | None = None) -> FreshnessRules | None:
    return None if "freshness" in config.agent.disabled_skills else load_configured_freshness_rules(config, store=store)
...
def create_skills(config: CommonConfig, *, handlers: SkillHandlers | None = None, store: EventStore | None = None, identity: RunIdentity | None = None, record_disclosures: bool | None = None, include_freshness: bool = True) -> Skills:
    return Skills(create_progressive_skill_disclosure(config, store=store, identity=identity, record_disclosures=record_disclosures, include_freshness=include_freshness), handlers)
