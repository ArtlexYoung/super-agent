...
def read_skill_change_report(data: dict[str, object]) -> "SkillChangeReport":
    ...
    return SkillChangeReport(str(data["report_id"]), str(data["change_id"]), float(data["score"]), None if data["baseline_score"] is None else float(data["baseline_score"]), bool(data["passed"]), float(data["minimum_score"]), bool(data["no_regression"]), None if data["improvement"] is None else float(data["improvement"]), float(data["minimum_improvement"]), bool(data["improvement_target_met"]), str(data["candidate_sha256"]), str(data["parent_sha256"]), str(data["created_at"]), results, baseline)
