...
from core.models import read_object, read_optional_int, read_text, read_text_list, reject_unknown_fields
...
class WebAPI:
    ...
    def handle(self, method: str, path: str, body: object | None = None) -> WebAPIResponse:
        ...
        if method == "GET" and parts[:2] == ["api", "runs"] and len(parts) == 3:
            return _ok(self.user.runs.explain(parts[2]))
        if method == "DELETE" and parts[:2] == ["api", "memory"] and len(parts) == 3:
            self.user.memory.forget(parts[2], "forgotten from web interface")
            return _ok(self._read_bootstrap())
        if method == "PUT" and path == "/api/config":
            self.user.configuration.update_agent_settings(read_agent_settings(body))
            return _ok(self._read_bootstrap())
        ...
        if method == "DELETE" and parts[:2] == ["api", "models"] and len(parts) == 3:
            self.user.skills.remove_model(parts[2])
            return _ok(self._read_bootstrap())
    ...
    def _read_bootstrap(self) -> dict[str, object]:
        ...
        return {"schema_version": 3, "agent": asdict(config.agent), "storage": {"backend": config.storage.backend, "path": str(config.storage.path), "audit": {"detailed_days": config.storage.audit.detailed_days, "critical_days": config.storage.audit.critical_days}}, "configuration_path": str(config.source), "skills": _web_skill_list(skill_index_to_dict(skills.index), config), "models": models, "conversations": [asdict(item) for item in self.user.conversations.list()], "runs": [asdict(item) for item in self.user.runs.list(50)], "memory": [asdict(item) for item in self.user.memory.list()], "subagents": _subagent_tree(self.agent, self.user_id, set(), [config.agent.name])}
...
def _web_skill_list(value: dict[str, object], config: CommonConfig) -> list[dict[str, object]]:
    ...
    return [{key: field for key, field in item.items() if key not in {"manifest_cache_path", "instructions_cache_path", "configuration_cache_path", "files_cache_path"}} | {"enabled": not {item["type"], item["key"], item["name"]} & disabled, "selected": item["key"] in selected or item["name"] in selected} for item in skills if isinstance(item, dict)]
