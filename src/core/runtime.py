...
from core.models import RunEvent, RunIdentity, RunResult, RuntimeEventSubscriberError, RuntimeEventSubscribers, Task
...
if TYPE_CHECKING:
    ...
    from core.records.store import DisclosureStorageFactory, EventStore, StorageBackend
...
class Run:
    ...
    def record_event(self, event_type: str, data: dict[str, object] | None = None) -> RunEvent:
        if self.task.subagent_record_options is not None:
            ...
            data = DEFAULT_AUDIT_POLICY.compact_event_data(event_type, data or {}, self.task.subagent_record_options)
    ...
    def close(self) -> None:
        ...
        for name, cleanup in reversed(cleanups):
            try:
                ...
                error.add_note(f"Run cleanup failed: {name}")
                errors.append(error)
        if len(errors) == 1: raise errors[0]
        if errors: raise ExceptionGroup("Run cleanup failed", errors)
...
def create_checkpoint_data(run_id: str, label: str, facts: dict[str, object]) -> dict[str, object]:
    ...
    if not clean_label: raise ValueError("checkpoint label cannot be empty")
    if not isinstance(facts, dict): raise TypeError("checkpoint facts must be a dictionary")
    try:
        encoded = _encode_checkpoint_value(facts)
    except (TypeError, ValueError) as error:
        raise ValueError("checkpoint facts must be JSON-compatible") from error
    if len(encoded) > CHECKPOINT_STATE_BYTES: raise ValueError("checkpoint facts exceed 16384 bytes")
...
def find_checkpoint_data(events: Iterable[RunEvent], checkpoint_id: str | None = None) -> dict[str, object]:
    ...
    if not checkpoints: raise KeyError("run has no checkpoints")
    if checkpoint_id is None: return checkpoints[-1]
    selected = next((item for item in checkpoints if item.get("checkpoint_id") == checkpoint_id.strip()), None)
    if selected is None: raise KeyError(f"checkpoint not found: {checkpoint_id}")
    return selected
    ...
    def run_task(self, request: Task, identity: RunIdentity, *, event_listener: Callable[[RunEvent], None] | None = None) -> RunResult:
        ...
        try:
            ...
            try:
                ...
                if run.task_runner is None: raise RuntimeError("run task loop is unavailable")
                result = run.task_runner.run_task(run)
            except BaseException as error:
                primary_error = error
                raise
            finally:
                _close_run(run, primary_error)
            ...
            raise
        result = replace(result, subscriber_failures=list(run.event_log.list_subscriber_failures()))
        ...
        final_result = replace(result, actions=list_run_actions(run), subscriber_failures=list(run.event_log.list_subscriber_failures()), events=run.event_log.list_events())
        failures = list(run.event_log.list_subscriber_failures())
        if failures and not request.allow_subscriber_failures: raise RuntimeEventSubscriberError(failures, final_result)
        return final_result
...
def _create_run(runtime: Runtime, request: Task, identity: RunIdentity, *, event_listener: Callable[[RunEvent], None] | None) -> Run:
    ...
    if runtime.storage is not None:
        ...
        store = EventStore(runtime.storage, runtime.config.storage.path, identity.user_id, identity.agent_name, run_event_log=event_log, disclosure_factory=runtime.disclosure_factory)
...
def _close_run(run: Run, primary_error: BaseException | None) -> None:
    try:
        run.close()
    except Exception as cleanup_error:
        if primary_error is None: raise
        primary_error.add_note(f"Run cleanup also failed: {type(cleanup_error).__name__}: {cleanup_error}")
