...
Replace `0.1.60` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.60 --full --web
```
...
git commit -m "refactor(cli): release v0.1.60 with one command vocabulary"
```
