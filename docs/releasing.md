...
Replace `0.1.54` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.54 --full --web
```
...
git commit -m "refactor(provider): release v0.1.54 with one pricing snapshot"
```
