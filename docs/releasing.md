...
Replace `0.1.38` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.38 --full --web
```
...
git commit -m "refactor(skill): release v0.1.38 with Skill-owned task work"
git tag v0.1.38
```
