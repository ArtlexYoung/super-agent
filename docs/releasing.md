...
Replace `0.1.68` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.68 --full --web
```
...
git commit -m "refactor(audit): release v0.1.68 with one event policy"
```
