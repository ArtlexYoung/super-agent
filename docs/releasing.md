...
Replace `0.1.37` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.37 --full --web
```
...
git commit -m "refactor(state): release v0.1.37 with clear state names"
git tag v0.1.37
```
