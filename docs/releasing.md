...
Replace `0.1.69` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.69 --full --web
```
...
git commit -m "refactor(api): release v0.1.69 with owned registration paths"
```
