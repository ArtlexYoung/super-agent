...
Replace `0.1.49` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.49 --full --web
```
...
git commit -m "refactor(adapter): release v0.1.49 with consolidated I/O owners"
```
