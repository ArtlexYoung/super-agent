...
Replace `0.1.24` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.24 --full --web
```
...
git commit -m "feat(runtime): release v0.1.24 with weighted priced subagents"
git tag v0.1.24
```
