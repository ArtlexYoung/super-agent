...
Replace `0.1.30` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.30 --full --web
```
...
git commit -m "refactor(skill): release v0.1.30 with one Skill runtime"
git tag v0.1.30
```
