...
Replace `0.1.63` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.63 --full --web
```
...
git commit -m "refactor(adapter): release v0.1.63 with user-domain I/O"
```
