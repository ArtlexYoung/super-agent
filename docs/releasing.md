...
Replace `0.1.56` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.56 --full --web
```
...
git commit -m "refactor(events): release v0.1.56 with one event pipeline"
```
