...
Replace `0.1.58` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.58 --full --web
```
...
git commit -m "refactor(skills): release v0.1.58 with one update transaction"
```
