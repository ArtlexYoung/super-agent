...
Replace `0.1.64` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.64 --full --web
```
...
git commit -m "refactor(runtime): release v0.1.64 with run resource snapshots"
```
