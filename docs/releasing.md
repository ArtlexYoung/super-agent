...
Replace `0.1.67` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.67 --full --web
```
...
git commit -m "refactor(storage): release v0.1.67 with shared SQL events"
```
