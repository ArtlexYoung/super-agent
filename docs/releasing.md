...
Replace `0.1.36` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.36 --full --web
```
...
git commit -m "refactor(disclosure): release v0.1.36 with one disclosure owner"
git tag v0.1.36
```
