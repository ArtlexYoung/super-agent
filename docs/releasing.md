...
Replace `0.1.39` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.39 --full --web
```
...
git commit -m "refactor(skill): release v0.1.39 with one file lifecycle owner"
git tag v0.1.39
```
