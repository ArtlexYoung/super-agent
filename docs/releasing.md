...
Replace `0.1.28` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.28 --full --web
```
...
git commit -m "refactor(provider): release v0.1.28 with one provider module"
git tag v0.1.28
```
