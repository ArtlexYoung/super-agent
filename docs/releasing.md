...
Replace `0.1.52` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.52 --full --web
```
...
git commit -m "refactor(tasks): release v0.1.52 with one group queue"
```
