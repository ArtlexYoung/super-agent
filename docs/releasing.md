...
Replace `0.1.25` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.25 --full --web
```
...
git commit -m "feat(runtime): release v0.1.25 with task-aware agent routing"
git tag v0.1.25
```
