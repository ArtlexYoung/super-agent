...
Replace `0.1.51` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.51 --full --web
```
...
git commit -m "refactor(tasks): release v0.1.51 with one task fact"
```
