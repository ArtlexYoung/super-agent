...
Replace `0.1.35` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.35 --full --web
```
...
git commit -m "refactor(boundary): release v0.1.35 with explicit Core ports"
git tag v0.1.35
```
