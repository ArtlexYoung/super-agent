...
Replace `0.1.43` with the version being released:

```bash
.venv/bin/python scripts/verify_release.py --version 0.1.43 --full --web
```
...
git commit -m "refactor(state): release v0.1.43 with one run state owner"
git tag v0.1.43
```
