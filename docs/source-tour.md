...
1. `src/super_agent.py`：查看 `Agent` 如何组合配置、模型、插件、Skill、存储、组和子 Agent。
...
6. `src/skill/document.py` 与 `src/skill/library.py`：查看标准 Skill 格式、插件去重、运行快照、索引和原子激活。
...
1. `src/super_agent.py`: how `Agent` composes configuration, models, plugins, Skills, storage, groups, and subagents.
...
6. `src/skill/document.py` and `src/skill/library.py`: the standard Skill format, plugin deduplication, run snapshots, indexes, and atomic activation.
